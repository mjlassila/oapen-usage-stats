...
Per subject group and language download data, using published

```{r}
knitr::kable(oapen_downloads_per_group)
```



Per subject group and language download data, using cleaned dataset
...
```{r publishing-profile-tup, message=FALSE, echo=FALSE}
knitr::kable(tup_fin %>% 
  group_by(classification) %>%
  mutate(n = n()) %>%
  summarize(
